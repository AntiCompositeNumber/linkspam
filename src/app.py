...
def linksearch():
    # Front page
    with open(
        os.path.join(app.config["linkspam_data_dir"], "linkspam_config.json")
    ) as f:
        data = json.load(f)
...
def api_report_target(target):
    try:
        with open(os.path.join(app.config["linkspam_data_dir"], target + ".json")) as f:
            data = json.load(f)
...
def linksearch_result(target):
    ...
    try:
        ...
        return flask.render_template("linkspam_noresult.html", target=target), 404
    else:
